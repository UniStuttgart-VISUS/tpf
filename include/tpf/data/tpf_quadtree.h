...
namespace tpf
{
    namespace data
    {
        ...
        class quadtree : public n_tree<float_t, value_t, geometry::rectangle<float_t, kernel_t>, 4>
        {
        public:
            using base_type = n_tree<float_t, value_t, geometry::rectangle<float_t, kernel_t>, 4>;
            using float_type = float_t;
            using store_type = std::pair<std::shared_ptr<geometry::rectangle<float_t, kernel_t>>, value_t>;
            using value_type = value_t;
            using kernel_type = kernel_t;

            /// <summary>
            /// Node of a tree, storing a value and up to four children
            /// </summary>
            ...
            public:
                ...
                virtual void set_children(const std::array<value_t, 4>& values) override;
            };
            ...
            quadtree(std::shared_ptr<node> root);
        ...
        protected:
            ...
            virtual std::array<neighbor_t, 3> get_directions(const geometry::point<float_t, kernel_type>& relative_point) const override;

            /// <summary>
            /// Get all potential directions, based solely on the kind of n-tree
            /// </summary>
            /// <returns>Neighboring directions</returns>
            virtual std::array<neighbor_t, 4> get_directions() const override;
        };
    }
}

#include "tpf_quadtree.inl"
