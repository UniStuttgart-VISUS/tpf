...
namespace tpf
{
    namespace geometry
    {
        ...
        template <typename floatp_t, typename kernel_t>
        inline std::shared_ptr<geometric_object<floatp_t>> triangle<floatp_t, kernel_t>::clone(const math::transformer<floatp_t, 3>& trafo) const
        {
            auto copy = std::make_shared<triangle<floatp_t, kernel_t>>(*this);
        ...
        template <typename floatp_t, typename kernel_t>
        inline geometric_object<floatp_t>& triangle<floatp_t, kernel_t>::transform(const math::transformer<floatp_t, 3>& trafo)
        {
            if (!trafo.is_unit())
            {
                this->_triangle = typename kernel_t::Triangle_3(
                    static_cast<point<floatp_t, kernel_t>&>(point<floatp_t, kernel_t>(this->_triangle.vertex(0)).transform(trafo)).get_internal(),
                    static_cast<point<floatp_t, kernel_t>&>(point<floatp_t, kernel_t>(this->_triangle.vertex(1)).transform(trafo)).get_internal(),
                    static_cast<point<floatp_t, kernel_t>&>(point<floatp_t, kernel_t>(this->_triangle.vertex(2)).transform(trafo)).get_internal());
            }
