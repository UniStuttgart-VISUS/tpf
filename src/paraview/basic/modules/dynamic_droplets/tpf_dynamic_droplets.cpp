...
namespace
{
    ...
    public:
        ...
        /// <param name="surface_alg">Algorithm producing droplet surfaces</param>
        ...
        data_handler(const std::size_t current_timestep, const std::vector<float_t>& timesteps,
            vtkInformation* request, vtkAlgorithm* droplets_alg, vtkAlgorithm* droplet_ids_alg, vtkAlgorithm* surface_alg,
            std::string translation_name, std::string rotation_name, std::string radius_name, std::string droplet_ids_name)
        {
        ...
        std::tuple<float_t, tpf::data::polydata<float_t>, tpf::data::grid<long long, float_t, 3, 1>, tpf::data::polydata<float_t>> operator()()
        {
            ...
            if (this->timesteps.size() > this->time_offset)
            {
                ...
                if (this->surface_alg != nullptr)
                {
                    this->surface_alg->Update(request);
                }
                ...
                if (this->surface_alg != nullptr)
                {
                    auto in_surface = vtkPolyData::SafeDownCast(this->surface_alg->GetOutputDataObject(0));
                    surface = tpf::vtk::get_polydata<float_t>(in_surface);
                }

                // Set time step delta and return
                float_t timestep_delta = this->timesteps[this->time_offset] - this->timesteps[this->time_offset - 1];

                return std::make_tuple(timestep_delta, droplets, droplet_ids, surface);
            }
...
int tpf_dynamic_droplets::RequestData(vtkInformation *request, vtkInformationVector **input_vector, vtkInformationVector *output_vector)
{
    try
    {
        ...
        auto in_surface = vtkPolyData::GetData(input_vector[2]);
        ...
        const auto surface = tpf::vtk::get_polydata<float_t>(in_surface);

        // Create output data
        tpf::data::polydata<float_t> droplet_tracks, summary, paths, axes, ribbons, rotation_paths, coordinate_axes;
        std::vector<tpf::data::polydata<float_t>> local_surfaces;
        ...
        if (this->Compute)
        {
            ...
            dynamic_droplets_module.set_input(droplets, droplet_ids, surface);
            dynamic_droplets_module.set_output(droplet_tracks, summary, paths, axes, ribbons, rotation_paths, coordinate_axes, local_surfaces);
            ...
            dynamic_droplets_module.set_parameters(static_cast<std::size_t>(this->NumTimeSteps), static_cast<float_t>(timestep),
                ...
                static_cast<tpf::modules::dynamic_droplets_aux::ribbon_duplication_t>(this->DuplicateRibbons), this->DuplicateTranslation,
                this->FixRotationAxisSize == 1, static_cast<float_t>(this->RotationAxisScale), this->RotationAxisTranslation,
            ...
            data_handler<float_t> call_back_loader(current_timestep, timesteps, request, GetInputAlgorithm(0, 0), GetInputAlgorithm(1, 0),
                GetInputAlgorithm(2, 0), translation_arr->GetName(), rotation_arr->GetName(), radius_arr->GetName(), ids_arr->GetName());
            ...
            // ... droplet surfaces over all time steps
            tpf::data::polydata<float_t> all_surfaces;

            for (const auto& local_surface : local_surfaces)
            {
                all_surfaces.merge(local_surface);
            }

            save_block<std::size_t, 1>(output, index++, all_surfaces, geometry_t::polygon, tpf::data::topology_t::CELL_DATA, "Time ID", "droplet surfaces");

            // ... droplet surfaces for each time step
            for (const auto& local_surface : local_surfaces)
            {
                auto block = vtkSmartPointer<vtkPolyData>::New();
                auto mesh = tpf::vtk::create_mesh(local_surface.get_geometry());

                block->SetPoints(mesh.points);
                block->SetPolys(mesh.polygonals);

                std::stringstream ss;
                ss << "droplet surface #" << (index - 8) << std::endl;

                output->SetBlock(index, block);
                output->GetMetaData(index)->Set(vtkCompositeDataSet::NAME(), ss.str().c_str());
